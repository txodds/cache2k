...
public class ClockProEvictionPreserveYoungerThanWithLimitTest extends TestingBase {

  protected Cache<Integer, Integer> provideCache(long size, long preserveMillis) {
    return builder(Integer.class, Integer.class).eternal(true).entryCapacity(size).recordModificationTime(true)
        .preserveYoungerThan(preserveMillis).preserveYoungerThanCapacityLimit(1).build();
  }
  ...
  public void testChunking() {
    ...
    Cache<Integer, Integer> c = provideCache(maxSize, 10000);
    int evictionChunk = 1;
    int previousSize = 0;
  ...
  public void test1() {
    final int size = 1;
    Cache<Integer, Integer> c = provideCache(size, 10000);
  ...
  public void testEvictCold() {
    final int size = 30;
    Cache<Integer, Integer> c = provideCache(size, 1000);
    for (int i = 0; i < size / 2; i++) {
      c.put(i, i);
    }
    for (int i = 0; i < size / 2; i++) {
  ...
  public void testChunkingExpired() {
    final int maxSize = 10000;
    final int minChunkSize = 1;
    Cache<Integer, Integer> c = provideCache(maxSize, 1);
    int evictionChunk = 1;
    int previousSize = 0;
    for (int i = 0; i < maxSize * 2; i++) {
      c.put(i, 1);
  ...
  public void test1Expired() {
    final int size = 1;
    Cache<Integer, Integer> c = provideCache(size, 1);
    for (int i = 0; i < size * 2; i++) {
      c.put(i, i);
      smallDelay();
    }
