...
public class ClockProEvictionPreserveNonExpiredWithLimitTest extends TestingBase {

  protected Cache<Integer, Integer> provideCache(long size) {
    return builder(Integer.class, Integer.class).eternal(true).entryCapacity(size).preserveNonExpired(true)
        .preserveNonExpiredCapacityLimit(1).build();
  }
  ...
  protected Cache<Integer, Integer> provideCacheExp(long size) {
    return builder(Integer.class, Integer.class).entryCapacity(size).expireAfterWrite(1, TimeUnit.MILLISECONDS)
        .sharpExpiry(false).keepDataAfterExpired(true).preserveNonExpired(true).preserveNonExpiredCapacityLimit(1).build();
  }
  ...
  public void testEvictHot2Expired() {
    ...
    assertEquals(14, countEntriesViaIteration());
    for (int i = size / 2; i < size; i++) {
      c.put(i, i);
      smallDelay();
    }
    ...
    assertEquals(0, countEntriesViaIteration());
  }
}
